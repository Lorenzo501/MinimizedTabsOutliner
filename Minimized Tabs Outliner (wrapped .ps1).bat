...
# this can likely be simplified for the specific use-case
...
# waiting on the user to log on so that WinSize2 becomes operational. not really necessary though b/c if tabs outliner gets opened later, it'll be operational.
# and WinSize2 uses a slower interval (that's good), which this script doesn't have to wait on if you let WinSize2 affect tabs outliner when you actually need it
while ($true)
{
...
<# OLD CODE (won't work when another app runs at startup and takes the first place in the ALT+TAB window collection)
...
# closing the chrome main window (can be done in AHK with this simple one-liner: WinClose("Nieuw tabblad - Google Chrome"))
$windows = [Win32.WindowList]::GetAllWindows()
$newTabChrome = $windows | where { $_.class -eq "Chrome_WidgetWin_1" -and $_.caption -eq "Nieuw tabblad - Google Chrome" }
$newTabChrome.Close()

<# if you don't want to minimize all windows; then get the handle that's saved in $newTabChrome and save it in $newTabChromeHandle, then the code below minimizes only that window
Add-Type -name NativeMethods -namespace Win32 -MemberDefinition @'
[DllImport("user32.dll")]
public static extern bool CloseWindow(IntPtr hWnd);
'@

[Win32.NativeMethods]::CloseWindow($newTabChromeHandle)
#>
