...
ChromeWatcher()
{
    static EVENT_MIN := 0x00000001, EVENT_MAX := 0x7FFFFFFF, hook := 0, cb := CallbackCreate(EventWatcher), index, logger

    if (hook = 0)
    {
        index := 0
        logger := FileOpen(A_Desktop "\logger.txt", 0x101)
...
F1::MsgBox("ChromeWatcher is " (ChromeWatcher() ? "enabled" : "disabled"),, 4096) ; Hook/unhook
F2::ProcessClose("chrome.exe") ; Closes all chrome windows (warning: chrome will think it crashed)
F3::
{
    MsgBox("Exiting",, 4096)
    ExitApp()
