...
ChromeWatcher(shouldRunChrome := false)
{
    ...
    {
        index := 0
        file := Logger("  # | Event      | Event Name                  | hWnd (hex) | hWnd (dec) | Window Title", true)
        hook := DllCall("SetWinEventHook", "UInt", EVENT_MIN, "UInt", EVENT_MAX, "Ptr", 0, "Ptr", cb, "UInt", 0, "UInt", 0, "UInt", 0, "Ptr")
        TrayTip("Hooked", "Chrome Watcher", 0x34)
    ...
    else
    {
        ...
        file.Close()
        hook := 0
        TrayTip("Unhooked", "Chrome Watcher", 0x34)
        SetTimer(HideTrayTip, -4000)
    }
...
Logger(text, shouldOpenFile := false)
{
    static file

    if (shouldOpenFile)
        file := FileOpen(A_Desktop "\logger.txt", 0x101)

    file.WriteLine(text)
    file.Read(0) ; Flush buffer (sometimes necessary to guarantee the proper write order)
...
F6::TrayTip("Exiting", "Chrome Watcher", 0x34), Sleep(4000), ExitApp()
