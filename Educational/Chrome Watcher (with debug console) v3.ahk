...
class ChromeWatcher
{
    ...
    HandleWinEvent(hWinEventHook, event, hWnd, *)
    {
        try if (WinGetProcessName(hWnd) = "chrome.exe")
        {
            ...
            Dbg.Instance.Update(++this.Index, Round((qpc - this.QpcPrevious) / this.Qpf * 1000), eventHex, eventName, hWnd, windowTitle?)
...
class Dbg
{
    ...
    __New()
    {
        ...
        this._Gui.AddButton(, "Hook/Unhook && Run Chrome").OnEvent("Click", (*) => (ChromeWatcher.Instance._Hook ? ChromeWatcher.Instance.Unhook() : ChromeWatcher.Instance.Hook(true), ControlClick(hiddenButton)))
        this._Gui.AddButton("x+6", "Hook/Unhook").OnEvent("Click", (*) => (ChromeWatcher.Instance._Hook ? ChromeWatcher.Instance.Unhook() : ChromeWatcher.Instance.Hook(false), ControlClick(hiddenButton)))
        this._Gui.AddButton("x+6", "Close Chrome").OnEvent("Click", (*) => (ProcessClose("chrome.exe"), ControlClick(hiddenButton)))
        this._Gui.AddButton("x+6", "Always On Top").OnEvent("Click", (*) => (Dbg.Instance.ToggleAlwaysOnTop(), ControlClick(hiddenButton)))
        this._Gui.AddButton("x+6", "Point Marker").OnEvent("Click", (*) => (ChromeWatcher.Instance._Hook ? Dbg.Instance.Update() : Exit(), ControlClick(hiddenButton)))
        ...
        ExportToTextFile()
        {
            ...
            FileAppend(Dbg.Instance.Text, filePath)
...
F1::(ChromeWatcher.Instance._Hook) ? ChromeWatcher.Instance.Unhook() : ChromeWatcher.Instance.Hook(true) ; Hook/unhook and runs chrome
F2::(ChromeWatcher.Instance._Hook) ? ChromeWatcher.Instance.Unhook() : ChromeWatcher.Instance.Hook(false) ; Hook/unhook
...
F4::Dbg.Instance.ToggleAlwaysOnTop() ; Toggle always-on-top
F5::(ChromeWatcher.Instance._Hook) ? Dbg.Instance.Update() : Exit() ; Adds a point marker/blank line
F6:: ; Exports to text file
{
    filePath := A_Desktop "\logger.txt"
    try FileDelete(filePath)
    FileAppend(Dbg.Instance.Text, filePath)
...
{
    selectedRowsCopy := Dbg.Instance.Header "`r`n"
    remainingRowsCopyAmount := Dbg.Instance.ListView.GetCount("Selected")
    selectedRow := Dbg.Instance.ListView.GetNext() + 1
    shouldSelectNextRow := false

    Loop Parse, Dbg.Instance.Text, "`n"
    {
